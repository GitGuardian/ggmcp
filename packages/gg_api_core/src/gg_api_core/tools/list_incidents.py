...
def _build_suggestion(params: "ListIncidentsParams", incidents_count: int) -> str:
    ...
    if params.severity:
        suggestions.append(f"Filtered by severity: {', '.join(params.severity)}")
...
class ListIncidentsParams(BaseModel):
    ...
    ordering: str | None = Field(
        default="-date",
        description="Sort field with optional '-' prefix for descending. Options: date, -date, severity, -severity, status, -status",
    )
...
async def list_incidents(params: ListIncidentsParams) -> ListIncidentsResult | ListIncidentsError:
    ...
    try:
        ...
        if params.severity:
            ...
            for sev in params.severity:
                if isinstance(sev, int):
                    ...
                    try:
                        severity_values.append(int(sev))
                    except ValueError:
                        severity_values.append(sev)
        ...
        source_ids = list(params.source_ids) if params.source_ids else []
        if params.repository_name:
