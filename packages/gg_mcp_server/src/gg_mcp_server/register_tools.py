...
def register_tools(mcp: AbstractGitGuardianFastMCP) -> None:
    ...
    mcp.tool(
        ...
        "repository in other internal-incident API calls. The server cannot access your local repository or run git: "
        "run `git config --get remote.origin.url` yourself and pass the result as the `remote_url` argument. "
